...
挑？
```
**长答拆轮规则**: 一题有 ≥3 子问题或 ≥3 列表项时, **Queen 自动判断是否需要拆轮**; 每轮 ≤3 段、≤6 行、≤400 字. 超过前先 split 到下一轮. 拆轮信号: (a) "列出 N 个 worker"; (b) "介绍 X"; (c) "头脑风暴"; (d) "深度思考" + 多分支. 拆轮策略: 第一轮答结论/子集, 第二轮补全/展开, 显式提示 "继续".
