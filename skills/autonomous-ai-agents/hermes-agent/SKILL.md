...
> **NOT a worker**: hermes-agent SKILL is the framework itself (CLI / TUI / gateway), not a worker skill.
> In SOUL §舰队表 L146, "hermes-agent 自身" refers to the **built-in `delegate_task` AIAgent child**
> — i.e. Queen spawning another hermes loop in-process via `delegate_task(tasks=[...])`.
> This is the 6th row of §舰队表, parallel to the 5 external CLI engines (codex/pi/opencode/shell/claude-code).
> See queen-dispatch/SKILL.md:190-195 for built-in vs external counter bridging.
